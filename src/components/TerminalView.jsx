...
export const TerminalView = ({ theme, themeMode }) => {
  ...
  const handleCommand = (e) => {
    ...
    switch(command) {
      ...
      case 'ls': 
      case 'projects': response = [{ type: 'info', content: 'ID              | NAME\n----------------+------------------' }, ...projects.map(p => ({ type: 'text', content: `${p.id.padEnd(15)} | ${p.title}` }))]; break;
      case 'view':
        const project = projects.find(p => p.id === argument);
        if (project) { response = [{ type: 'success', content: `>> Opening ${project.title}...` }, { type: 'info', content: `Tech: ${project.tech.join(', ')}` }, { type: 'text', content: project.details }]; }
  ...
  return (
    <div className="h-full flex items-center justify-center p-4 md:p-8 animate-in fade-in zoom-in duration-500">
      <div className={`w-full max-w-4xl ${theme.bgSoft}/90 backdrop-blur-xl rounded-xl border ${theme.border} shadow-2xl flex flex-col h-[70vh] md:h-[600px]`}>
        <div className={`bg-slate-950/20 px-4 py-3 flex justify-between border-b ${theme.border}`}>
        ...
        <div className="flex-1 p-6 overflow-y-auto font-mono text-sm md:text-base custom-scrollbar" onClick={() => inputRef.current?.focus()}>
          {history.map((line, i) => (
            <div key={i} className="mb-2 break-words whitespace-pre-wrap"> 
              {line.type === 'cmd' ? <div className="flex"><span className={`mr-2 ${successColor}`}>➜</span><span className={theme.text}>{line.content}</span></div> : <div className={theme.text}>{line.content}</div>}
            </div>
          ))}
